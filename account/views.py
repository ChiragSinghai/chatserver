...
def get_redirect_if_exists(request):
    ...
    if request.GET:
        if request.GET.get("next"):
            redirects = str(request.GET.get("next"))
    return redirects
...
def edit_account_view(request, *args, **kwargs):
    ...
    if request.POST:
        ...
        if form.is_valid():
            if account.profile_image and account.profile_image != request.user.profile_image and account.profile_image != get_default_profile_image():
                account.profile_image.delete()
            form.save()
            return redirect("account:view", user_id=account.pk)
        else:
            form = AccountUpdateForm(request.POST, instance=request.user, initial={"id": account.pk,
                                                                                   "email": account.email,
...
def crop_image(request, *args, **kwargs):
    ...
    if request.POST and user.is_authenticated:
        try:
            ...
            if user.profile_image != 'static/default_profile_image.png':
